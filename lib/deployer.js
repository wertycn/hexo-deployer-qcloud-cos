...
function getConfig(args, hexo) {
    ...
    const config = {
        ...
        remoteDir   : remoteDir,
        bucketAcl   : args.bucketAcl || 'private',
...
function checkConfig(config) {
    if (!config.SecretKey || !config.SecretId || !config.Bucket || !config.Appid || !config.Region) {
        ...
        help += '如需更多帮助，您可以查看文档：' + chalk.underline('http://github.com/wertycn/hexo-deployer-qcloud-cos.html');
        console.log(help);
        return false;
    } else {
        return true;
...
function createBucket(client,config){
    ...
    client.putBucket(params, function(err, data) {
        if(err) {
            ...
            console.log("bucket %s 创建成功! 默认您需要在腾讯云控制台查看默认域名及添加自定义域名后即可正常访问博客，更多信息请查看%s" ,config.Bucket,chalk.underline("http://github.com/wertycn/hexo-deployer-tencent-cos.html"));
            console.log(data)
            // 控制台交互
            inquirer.prompt([{
                type: 'confirm',
...
function deploy(client, config) {
    if (!config.Appid) {
        console.log("您没有设置Appid 无法进行bucket验证")
        return false
    }
    const params = {
