        ...
        client.putObject({
            ...
            if (err) {
                ...
                if (data.statusCode == 200) {
                    ...
                    let logFilePath = __dirname + '/logs.json'
                    let logs = JSON.parse(fs.readFileSync(logFilePath))
                    logs[localPath] = stat.mtime.getTime()
...
function uploadPublicFiles(client,config){
    ...
    try{
        ...
        // 失败就重新生成日志文件
        fs.writeFileSync(__dirname + '/logs.json','{}')
    }
    
    rd.eachFileSync(config.localDir, function (f) {
        // 每找到一个文件都会调用一次此函数
        let stat = fs.statSync(f);
        if(!logs[f] || logs[f] != stat.mtime.getTime()) {
            put(client, config, f)
        }else{
            console.log("[%s] 跳过:%s", chalk.green('Deployer info'), f)
        }
    });
}
